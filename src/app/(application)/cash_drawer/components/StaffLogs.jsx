...
export function StaffDrawerLogsTable({ logs = [] }) {
	...
	const columns = [
		...
		{
			id: 'cash_in_drawer',
			accessorKey: "cash_in_drawer",
			header: "Cash In Drawer",
			cell: ({ row }) => (
				<div>
					Rs. {row.original.cash_in_drawer.toLocaleString() || 0}
				</div>
			),
		},
		{
			id: 'expected_cash_in_drawer',
			accessorKey: "expected_cash_in_drawer",
			header: "Estimated Amt.",
			cell: ({ row }) => (
				<div>
					Rs. {row.original.expected_cash_in_drawer.toLocaleString() || 0}
				</div>
			),
		},
		{
			id: 'controller_count',
			accessorKey: "controller_count",
			header: "Controllers",
			cell: ({ row }) => (
				<div>
					{row.original.controller_count.toLocaleString() || 0}
				</div>
			),
		},
		{
		...
		},
		{
			...
			cell: ({ row }) => (
				<div>
					{row.original.expand?.user_id?.username}
				</div>
			),
		},
	];
